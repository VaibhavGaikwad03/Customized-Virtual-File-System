    ...
    while (temp != NULL)
    {
        if (temp->file_type != 0)
        {
            ...
            if (fd == -1)
                ...
                write(fd, temp->file_data, FILE_SIZE);
        }
        temp = temp->next_inode;
    }
}
...
int close(char *file_name)
{
    ...
    ufdt_array[fd].ptr_filetable->ptr_inode->reference_count = 0;
...
int create_file(char *file_name, int permission)
{
    ...
    memset(new_inode->file_data, 0, FILE_SIZE);
