...
void close_all_files()
{
    ...
    for (counter = 0; counter < MAX_INODES; counter++)
    {
        ...
        {
            (ufdt_array[counter].ptr_filetable->ptr_inode->reference_count)--;

            if ((ufdt_array[counter].ptr_filetable->ptr_inode->reference_count == 0)) // if reference count 0 free file table entry
            {
...
int close_file(char *file_name)
{
    ...
    if (filetable_ptr->reference_count == 0)
    {
        ...
        ufdt_array[file_desc].ptr_filetable = NULL; // most important (dependency in get_free_inode())
...
int write_file(int file_desc, char *file_data, int no_of_bytes)
{
    return 0;
}
